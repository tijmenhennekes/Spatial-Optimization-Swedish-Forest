...
# ---- Current management overlay: carbon reference and WTP budget ----
...
# 2030 target used only for REPORTING here (carbon is not constrained in S1).
carbon_target_Mt <- current_mgmt_carbon_Mt * (47.321 / 43.366)   # corrected LULUCF ratio
...
# ------------------------------ CONSTRAINTS -----------------------------------
# Existing protection is locked in (reserves cannot be un-protected).
...
policy_problem <- prioritizr::problem(cost_stack, feature_biodiversity) %>%
  ...
  # 30% CN floor (EU minimum) kept; upper cap removed, so CN may exceed 30%.
...
cat("\n========  LAUNCHING GUROBI - SCENARIO 1 (optimisation at current budget)  ========\n")
...
cat("\n--- SECURED COVERAGE BY IUCN TIER (S1) ---\n"); print(tier_tbl, row.names = FALSE)

# --- CARBON (reported vs the 2030 target; NOT constrained) ---
cat("\n--- CARBON (relative, reported) ---\n")
...
cat(sprintf("S1 carbon: %.2f Mt  |  vs 2030 target %.2f Mt: %.1f%%\n", c_tot, carbon_target_Mt, 100*c_tot/carbon_target_Mt))
...
cat(sprintf("\nS1 cost: %.1f  |  WTP budget: %.1f  |  utilisation: %.1f%%\n",
            solution_cost, solver_budget, 100 * solution_cost / solver_budget))

# --- EXPORT ---
terra::writeRaster(solution_map, file.path(out_dir, "scenario1_solution_zones.tif"), overwrite = TRUE)
tryCatch(writexl::write_xlsx(rep_species, file.path(out_dir, "scenario1_representation_secured.xlsx")),
         error = function(e) cat("WARNING: xlsx not written -", conditionMessage(e), "\n"))
cat("\nScenario 1 results saved to:", out_dir, "\n")